...
type RefreshTokenEndLifeCallbackType = (
	rbAuthErros: RBAuthErrors,
	error?: Error
) => void;

const dummyFunction = (arg: unknown) =>
	new Promise<unknown>((resolve) => resolve(arg));
	...
	implements AuthApiInterface
{
	...
	constructor(
		...
		if (refreshTokenEndLifeCallback)
			this.errorCallback = refreshTokenEndLifeCallback;
	...
	login: AuthProcessResponse = async (...args: any) =>
		this.runLogic(this.loginLogic)(...args);
	handle: AuthProcessResponse = async (...args: any) =>
		this.runLogic(this.handleLogic)(...args);
	silent: AuthProcessResponse = async (...args: any) =>
		this.runLogic(this.silentLogic)(...args);
	// simple signup
	signup: UnknownAuthProcess = (...args: any) => this.signupLogic(...args);
	// custom logic
	refresh: RefreshProcessResponse = async (f: boolean) =>
		this.runRefresh(this.refreshLogic)(f);
	...
	private embedWrapperLogicIntoApis(apis: AppApis) {
		...
		Object.keys(apis).forEach((item) => {
			if (
				item.startsWith("logic") &&
				apis[item] &&
				typeof apis[item] === "function"
			)
	...
	private runRefresh =
		(logic: RefreshProcessResponse) => async (flag: boolean) =>
			this.wrap(async () => {
	...
	private wrap = async <T>(logic: () => T, flag = true) => {
		...
		try {
			...
			if (e.message === "Failed to fetch")
				this.errorCallback(RBAuthErrors.FAILLED_TO_FETCH, e);
			// others
